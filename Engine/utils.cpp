...
bool utils::IsOverlapping(const std::vector<Point2f>& vertices, const Rectf& rect)
{
	...
	const Point2f p2{ rect.left + rect.width, rect.bottom };
	const Point2f p3{ rect.left + rect.width, rect.bottom + rect.height };
	const Point2f p4{ rect.left, rect.bottom + rect.height };

	return IsPointInPolygon(p1, vertices) && IsPointInPolygon(p2, vertices) && IsPointInPolygon(p3, vertices) && IsPointInPolygon(p4, vertices);
}
